...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const extname = allowedTypes.test(file.originalname.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp)$/));
...
router.post('/upload', upload.array('images', 12), async (req, res) => {
  try {
    ...
    const uploadedImages = req.files.map(file => ({
      ...
      description: '',
      publicId: file.filename // Cloudinary public ID for deletion if needed
    }));
